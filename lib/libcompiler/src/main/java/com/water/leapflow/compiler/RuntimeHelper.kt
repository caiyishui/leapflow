package com.water.leapflow.compiler
...
object RuntimeHelper {
    ...
    fun init(env: ProcessingEnvironment){
        ...
        options.keys.forEach {
            println("RuntimeHelper ProcessingEnvironment ${it}  ${options[it]}")
        }
        var path =options["kapt.kotlin.generated"]?.split("/")?.last()
        println("RuntimeHelper ProcessingEnvironment =====  ${path}")
        kotlin.runCatching {
        ...
        val projectPath = outputPath.replace("${File.separator}build${File.separator}generated${File.separator}source${File.separator}kapt${File.separator}${path}${File.separator}$TASK_FILE","")
